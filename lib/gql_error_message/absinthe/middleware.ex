  ...
  *after* the `resolve` function.
  ...
  def call(
    ...
    case errors
      ...
      [] ->
        raise "GQLErrorMessage failed to translate errors\n\nerrors: #{inspect(errors)}"
    end
  end
  ...
  defp to_jsonable_map(errors, opts) when is_list(errors) do
    Enum.map(errors, &to_jsonable_map(&1, opts))
  end

  defp to_jsonable_map(%ClientError{} = e, opts) do
    ClientError.to_jsonable_map(e, opts)
  end

  defp to_jsonable_map(%ServerError{} = e, opts) do
    ServerError.to_jsonable_map(e, opts)
  end
end
