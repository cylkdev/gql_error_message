defmodule GQLErrorMessage.Absinthe.PerFieldMiddlewareTest do
  use ExUnit.Case, async: true

  alias GQLErrorMessage.Support.Absinthe.PerFieldSchema
  ...
  describe "query success" do
    test "returns data with no errors" do
      assert {:ok, result} = Absinthe.run(@user_query, PerFieldSchema, variables: %{"name" => "valid"})
  ...
  describe "query client error" do
    test "promotes client error to server error in top-level errors" do
      assert {:ok, result} = Absinthe.run(@user_query, PerFieldSchema, variables: %{"name" => "not_found"})
  ...
  describe "query server error" do
    test "returns server error in top-level errors" do
      assert {:ok, result} = Absinthe.run(@user_query, PerFieldSchema, variables: %{"name" => "server_error"})
  ...
  describe "query unhandled error" do
    test "raises RuntimeError for unrecognized error types" do
      assert_raise RuntimeError, ~r/Unsupported error type/, fn ->
        Absinthe.run(@user_query, PerFieldSchema, variables: %{"name" => "unhandled"})
      end
    end
  end
  ...
  describe "query changeset error" do
    test "promotes changeset client error to server error" do
      assert {:ok, result} = Absinthe.run(@user_query, PerFieldSchema, variables: %{"name" => "changeset"})
  ...
  describe "mutation success" do
    test "returns data with empty user_errors" do
      ...
      assert {:ok, result} =
               Absinthe.run(@create_user_mutation, PerFieldSchema, variables: %{"input" => input})
  ...
  describe "mutation changeset error" do
    ...
    test "returns nested changeset errors with string messages" do
      input = %{"name" => "nested_changeset", "email" => "test@test.com"}
